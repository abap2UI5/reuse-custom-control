...
// @abap2ui5/embed-control was built from, or a local checkout named by
// ABAP2UI5_DIR (to try a change before it is merged).
//
// ONE pin for everything: examples/host-app/package.json names the exact
// @abap2ui5/embed-control version the example depends on, and that package
...
// job builds the backend from it - so the frontend-cc branches and the tests
// cannot pair different frontends. There used to be a second
...
// The abap2UI5 commit of the installed @abap2ui5/embed-control - resolved from
// the workspace, so `npm ci` has to have run.
export function readPin() {
  let manifest;
  try {
    manifest = createRequire(join(root, "package.json"))(
      "@abap2ui5/embed-control/package.json",
    );
  } catch {
    throw new Error(
      "@abap2ui5/embed-control is not installed - run npm ci first (its version in " +
        "examples/host-app/package.json is the abap2UI5 release everything here builds from)",
    );
  }
  const pin = manifest.abap2ui5?.commit ?? "";
  if (!/^[0-9a-f]{40}$/.test(pin)) {
    throw new Error(
      `@abap2ui5/embed-control@${manifest.version} records no abap2UI5 commit: '${pin}'`,
    );
  }
  return pin;
}
    ...
    if (head !== pin) {
      console.warn(
        `${label}: WARNING - using ${dir} at ${head.slice(0, 8)}, ` +
          `@abap2ui5/embed-control is built from ${pin.slice(0, 8)}. Do not deploy this build.`,
      );
    }
    return dir;
  }
